...
function buildExtractSystemPrompt(isUsingPrintExtractedDataTool = false, useTextExtract = false, userProvidedInstructions) {
...
ONLY print the content using the print_extracted_data tool provided.
  ...
  const additionalInstructions = useTextExtract ? `Once you are given the text-rendered webpage, 
    ...
    do not miss any important information.` : "If a user is attempting to extract links or URLs, you MUST respond with ONLY the IDs of the link elements. \nDo not attempt to extract links directly from the text unless absolutely necessary. ";
...
function transformSchema(schema, currentPath) {
  ...
  if (isKind(schema, import_zod3.ZodFirstPartyTypeKind.ZodString)) {
    ...
    if (hasUrlCheck) {
      return [makeIdNumberSchema(schema), [{ segments: [] }]];
    }
    return [schema, []];
  }
  ...
  if (obj && typeof obj === "object") {
    ...
    if (path4.length === 1) {
      ...
      if (typeof fieldValue === "number") {
        ...
        record[key] = mappedUrl != null ? mappedUrl : ``;
      }
    } else {
...
function makeIdNumberSchema(orig) {
  var _a15, _b, _c;
  const userDesc = (
  ...
  const base = "This field must be filled with the numerical ID of the link element";
  const composed = userDesc.trim().length > 0 ? `${base} that follows this user-defined description: ${userDesc}` : base;
  return import_zod3.z.number().describe(composed);
}
...
function formatSimplifiedTree(node, level = 0) {
  ...
  let result = `${indent}[${node.nodeId}] ${node.role}${cleanName ? `: ${cleanName}` : ""}
`;
...
function getAccessibilityTree(page, logger, selector) {
  return __async(this, null, function* () {
    var _a15;
    yield page.enableCDP("Accessibility");
    try {
      const { nodes: fullNodes } = yield page.sendCDP(
        "Accessibility.getFullAXTree"
      );
      const scrollableBackendIds = yield findScrollableElementIds(page);
      let nodes = fullNodes;
      if (selector) {
        const objectId = yield resolveObjectIdForXPath(page, selector);
        const { node } = yield page.sendCDP("DOM.describeNode", { objectId });
        ...
        while (queue.length) {
          ...
          for (const childId of (_a15 = current.childIds) != null ? _a15 : []) {
            if (!keep.has(childId)) {
              keep.add(childId);
        ...
        nodes = fullNodes.filter((n) => keep.has(n.nodeId)).map(
      ...
      const hierarchicalTree = yield buildHierarchicalTree(
        nodes.map((node) => {
          var _a16, _b, _c, _d;
          let roleValue = ((_a16 = node.role) == null ? void 0 : _a16.value) || "";
...
function findScrollableElementIds(stagehandPage) {
  return __async(this, null, function* () {
    ...
    for (const xpath of xpaths) {
      if (!xpath) continue;
      const objectId = yield resolveObjectIdForXPath(stagehandPage, xpath);
      if (objectId) {
...
function resolveObjectIdForXPath(page, xpath) {
  return __async(this, null, function* () {
    const { result } = yield page.sendCDP("Runtime.evaluate", {
      expression: `
      (function () {
            ...
            this.page.evaluate(() => {
              return new Promise((resolve2) => {
                if (typeof window.waitForDomSettle === "function") {
                  window.waitForDomSettle().then(resolve2);
                } else {
                  console.warn(
  ...
  createChatCompletion(_0) {
    return __async(this, arguments, function* ({
      ...
      if (options.response_model) {
        const response2 = yield (0, import_ai2.generateObject)({
          model: this.model,
        ...
        const result2 = {
          ...
          usage: {
            prompt_tokens: (_d = response2.usage.promptTokens) != null ? _d : 0,
            completion_tokens: (_e = response2.usage.completionTokens) != null ? _e : 0,
            total_tokens: (_f = response2.usage.totalTokens) != null ? _f : 0
          }
        };
        ...
        (_h = this.logger) == null ? void 0 : _h.call(this, {
          category: "aisdk",
          message: "response",
          level: 2,
          auxiliary: {
            response: {
              value: JSON.stringify(response2),
              type: "object"
            },
            requestId: {
              value: options.requestId,
      ...
      const tools = {};
      for (const rawTool of (_i = options.tools) != null ? _i : []) {
        tools[rawTool.name] = {
          description: rawTool.description,
          parameters: rawTool.parameters
        };
      }
      const response = yield (0, import_ai2.generateText)({
        model: this.model,
        messages: formattedMessages,
        tools
      });
      const result = {
        data: response.text,
        usage: {
          prompt_tokens: (_j = response.usage.promptTokens) != null ? _j : 0,
          completion_tokens: (_k = response.usage.completionTokens) != null ? _k : 0,
          total_tokens: (_l = response.usage.totalTokens) != null ? _l : 0
        }
      };
      if (this.enableCaching) {
      ...
      try {
        switch (action.type) {
          case "click": {
            ...
            const newOpenedTab = yield Promise.race([
              new Promise((resolve2) => {
                this.stagehandPage.context.once("page", (page) => resolve2(page));
                setTimeout(() => resolve2(null), 1500);
              })
            ]);
            if (newOpenedTab) {
  ...
  return __async(this, null, function* () {
    ...
    if (env === "BROWSERBASE") {
      ...
      if (localBrowserLaunchOptions == null ? void 0 : localBrowserLaunchOptions.cdpUrl) {
        ...
        const browser2 = yield import_test2.chromium.connectOverCDP(
